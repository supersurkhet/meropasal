	...
	{:else}
				...
				<Table>
					...
					<TableBody>
						...
						{:else}
							...
							{#if vItems.length > 0}
								<tr><td colspan="4" style="height:{vItems[0].start}px;padding:0;border:none;"></td></tr>
							{/if}
							{#each vItems as vRow (vRow.key)}
								{@const vehicle = filteredVehicles[vRow.index]}
							...
							{/each}
							{#if vItems.length > 0}
								<tr><td colspan="4" style="height:{totalSize - vItems[vItems.length - 1].end}px;padding:0;border:none;"></td></tr>
							{/if}
						{/if}
					</TableBody>
				</Table>
		...
		{:else}
			<div class={viewPref.mode === 'grid-3'
				...
				{:else}
					{@const vItems = $virtualizer.getVirtualItems()}
					{@const chunks = chunkItems(filteredVehicles, getLanes())}
					{@const padTop = vItems.length > 0 ? vItems[0].start : 0}
					{@const padBottom = vItems.length > 0 ? $virtualizer.getTotalSize() - vItems[vItems.length - 1].end : 0}
					<div style="padding-top:{padTop}px;padding-bottom:{padBottom}px;">
						<div class={viewPref.mode === 'grid-3'
